...
# This script generates, solves and plots a large-scale quadratically constrained quadratic program,
# corresponds to results presented in Section 5.4


if __name__ == "__main__":
    plots = True  # generate and save all plots?
    cvx_bool = False  # comparison with cvxpy results (only seems to work if solution is not 0)?
    ...
    # build problem
    qp = QuadraticProgram(n=n, m=m, density=n**(-9/10))
    np.random.seed(2206)
    qp.generate_problem()
    ...
    np.save('../results/qcqp_large_log_outer', p.log_outer)
    np.save('../results/qcqp_large_log_inner', p.log_inner)
    np.save('../results/qcqp_large_qp', qp)
    ...
    if plots:
        generate_all_plots(p, 'qcqp_large', fstar=fstar, xstar=xstar, cliplast=500-212, show=False)
