...
# This script generates, solves and plots a small-scale geometric program,
# corresponds to results presented in Section 5.3


if __name__ == "__main__":
    plots = False  # generate and save all plots?
    cvx_bool = True  # comparison with cvxopt results (only seems to work if solution is not 0)?
    ...
    np.save('../results/gp_small_log_outer', p.log_outer)
    np.save('../results/gp_small_log_inner', p.log_inner)
    np.save('../results/gp_small_gp', gp)
    ...
    if plots:
        generate_all_plots(p, 'gp_small', fstar=fstar, xstar=xstar, show=False)
